...
from federatedscope.contrib.utils.utils import seed_everything
# import os
...
#         model.eval()
#         with torch.no_grad():
#             logits,_ = model.forward((x, edge_index))
...
#         device = y_pred.device
#
#         # Ensure edge_index and y_pred are on the same device
#         edge_index = edge_index.to(device)
#         y_pred = y_pred.to(device)
#
#         # Initialize tensor to hold edge destination classes
#         edge_dest_class = torch.zeros(
#             (n_edge, n_class), dtype=torch.float32, device=device
#         ).scatter_(
#             1, y_pred[edge_index[1]].unsqueeze(1), 1
#         )  # [n_edges, n_class]
#
#         # Initialize tensor to hold neighbor label distribution
#         neighbor_y_distr = torch.zeros(
#             (n_node, n_class), dtype=torch.float32, device=device
#         ).scatter_add_(
#             0,
#             edge_index[0].unsqueeze(1).expand(-1, n_class),
#             edge_dest_class
#         )  # [n_nodes, n_class]
#
#         # Row-wise normalization with handling division by zero
#         row_sums = neighbor_y_distr.sum(axis=1, keepdim=True)
#         row_sums[row_sums == 0] = 1  # Avoid division by zero
#         neighbor_y_distr /= row_sums
#
#         self.time_neighbor_distr = time.time() - start_time
#         return neighbor_y_distr
#
...
#         # print("y_pred" ,y_pred)
#         # print("n_classes" ,self.n_class)
#         # print("len weights" , len(self.train_class_weights))
#         # print("train_class_weight:", self.train_class_weights)
#         # node_risk *= self.train_class_weights[y_pred]
#         # self.time_risk_comp = time.time() - start_time
#         # return node_risk
#         device = y_pred_proba.device
#         # compute node pred
#         start_time = time.time()
#         node_unc = 1 - y_pred_proba.max(axis=1).values
#         self.time_unc_comp = time.time() - start_time
...
#         node_unc_class_mean = self.get_group_mean(node_unc, y_pred, self.classes)
#         node_risk = (node_unc - node_unc_class_mean).clip(min=0)
#         # Use a defaultdict to provide a default weight for missing classes
#         default_weight = 1.0  # Change this value to your preferred default weight
#         class_weights = defaultdict(lambda: default_weight, enumerate(self.train_class_weights))
#         # print("y_pred" ,y_pred)
#         # print("n_classes" ,self.n_class)
#         # print("len weights" , len(self.train_class_weights))
#         # print("train_class_weight:", self.train_class_weights)
#         # Apply class weights to node risk
#         weights_tensor = torch.tensor([class_weights[int(cls)] for cls in y_pred], dtype=torch.float32).to(device)
#         node_risk *= weights_tensor
#         # print("node_risk:", node_risk)
#         self.time_risk_comp = time.time() - start_time
#
#         return node_risk
#
...
class BatAugmenter(BaseGraphAugmenter):
    ...
    def init_with_data(self, data: pyg.data.Data, client_id):
        """
        Initializes the augmenter with graph data.
        ...
        all_classes = torch.arange(self.n_class, device=device)
        missing_classes = all_classes[~torch.isin(all_classes, classes)]

        # if client_id == 1:
        #     new_element = torch.tensor([6], dtype=missing_classes.dtype, device=missing_classes.device)
        #     missing_classes = torch.cat([missing_classes, new_element])

        if len(missing_classes) > 0:
            print(f"Missing classes: {missing_classes}")
            # print("pre_x:",len(data.x))
            # Create virtual nodes for missing classes
            virtual_nodes = torch.zeros((len(missing_classes), x.shape[1]), device=device)
            # virtual_y = torch.tensor(missing_classes, device=device)
            virtual_y = missing_classes.clone().detach()
            virtual_train_mask = torch.ones(len(missing_classes), dtype=torch.bool, device=device)

            # Concatenate virtual nodes with the original data
            x = torch.cat([x, virtual_nodes], dim=0)
            y = torch.cat([y, virtual_y], dim=0)
            y_train = torch.cat([y_train, virtual_y], dim=0)
            train_mask = torch.cat([train_mask, virtual_train_mask], dim=0)

            virtual_edges = torch.stack(
                [torch.arange(self.n_node, self.n_node + len(missing_classes), device=device)] * 2)
            edge_index = torch.cat([edge_index, virtual_edges], dim=1)

            # Update the statistics

            self.n_node = x.shape[0]
            self.classes, self.train_class_counts = y_train.unique(return_counts=True)
            self.n_class = len(self.classes)
            self.y_virtual = self.classes
            self.y_train = y_train
            self.train_mask = train_mask
            self.train_class_weights = self.train_class_counts / self.train_class_counts.max()
    ...
    def augment(
        ...
        # Use updated x and edge_index if not provided
        if x is None:
            x = self.x
        ...
        y_pred_proba = self.predict_proba(model, x, edge_index)
        # print("y_pred[train_mask]:", len(y_pred_proba))
