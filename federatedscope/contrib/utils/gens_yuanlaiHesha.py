...
def sampling_idx_individual_dst(class_num_list, idx_info, device):#每个类的节点数list7，每个类对应的训练索引构成的7个tensor
    ...
    for cls_idx, samp_num in zip(idx_info, sampling_list):
        if len(cls_idx) > 0:
            sampled_indices = cls_idx[torch.randint(len(cls_idx), (int(samp_num.item()),))]
            sampling_src_idx.append(sampled_indices)
        else:
            continue
            # 处理 cls_idx 为空的情况，可以采取适当的措施
    sampling_src_idx = torch.cat(sampling_src_idx).long()
    ...
    sampling_src_idx, sorted_idx = torch.sort(sampling_src_idx)#第一个返回值是排序后的张量，包含了按升序排列的元素。第二个返回值是一个索引张量，它包含了排序后的张量元素在原始张量中的索引位置。
...
def saliency_mixup(x, sampling_src_idx, sampling_dst_idx, lam):
    ...
    """
    new_src = x[sampling_src_idx.to(x.device), :].clone()#获得x中sampling_src_idx索引节点的特征
...
@torch.no_grad()
def sampling_node_source(class_num_list, prev_out_local, idx_info_local, train_idx, tau=2, max_flag=False, no_mask=False):# class_num_list是训练集每个类的节点个数
    max_num, n_cls = max(class_num_list), len(class_num_list) 
    ...
    for cls_idx, num in enumerate(sampling_list):
        ...
        if num <= 0: 
            continue
        #如果有一类的训练集无节点，则不采样这个类为anc
        if class_num_list[cls_idx] ==0:
            continue
        # first sampling --vanc
        ...
        if not no_mask:
            conf_src[:,cls_idx] = 0
        # #如果有一类的训练集无节点，则不采样这个类为aux
        for cls in range(len(class_num_list)):
            if class_num_list[cls] ==0:
                conf_src[:, cls] = 0

        neighbor_cls = []
        for row in conf_src:
            if sum(row)<1e-8:#置信度阈值
                cls = cls_idx
            else:
                cls = torch.multinomial(row+ 1e-12, 1)
    ...
    return src_idx_all, dst_idx_all #vanc,vaux
