...
#
#     # 计算每个节点的总连接权重（节点体积）
#     node_vol = adj_matrix.sum(axis=1)
#
#     # 计算所有节点体积的总和
#     VOL = node_vol.sum()
#
#     # 创建邻接列表
#     adj_list = [set(np.nonzero(adj_matrix[i])[0]) for i in range(g_num_nodes)]
#
#     return g_num_nodes, VOL, node_vol.tolist(), adj_list
...
class PartitionTree():
    ...
    #     node_dict[new_id].child_h = node_dict[node_id].child_h + 1
    #
    #     # Create a copy of the adjacency list to iterate over
    #     adj_list_copy = list(self.adj_table[node_id])
    #
    #     self.adj_table[new_id] = self.adj_table[node_id]
    #     for i in adj_list_copy:
    #         self.adj_table[i].add(new_id)
