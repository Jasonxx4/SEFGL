...
METRIC_NAME = 'pre_based_on_global_prototype'


def pre_based_on_global_prototype(ctx, **kwargs):
    loss_mse = nn.MSELoss()
    ...
    pre = precision_score(labels.cpu(),pred_labels.cpu(),average="macro",zero_division=1)
    return pre

def call_my_metric(types):
    if METRIC_NAME in types:
        the_larger_the_better = True
        metric_builder = pre_based_on_global_prototype
