    ...
    def _check_and_parse(self, ctx):
        ...
        if ctx.get('ys_pred', None) is not None:
            ...
            if 'regression' in ctx.cfg.model.task.lower():
                ...
                valid_pred_mask = np.in1d(y_pred, unique_classes)
                if not np.all(valid_pred_mask):
                    # 寻找最频繁的有效类别
                    most_frequent_valid_class = np.bincount(y_true.flatten()).argmax()

                    # 替换无效预测为最频繁的有效类别
                    y_pred[~valid_pred_mask] = most_frequent_valid_class
