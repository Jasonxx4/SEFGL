...
class GPUManager():
    ...
    #         self.gpus = self._query_gpus()
    #         for gpu in self.gpus:
    #             self.devices.append('cuda:{}'.format(gpu['index']))
    #     else:
    #         self.gpus = None
    ...
    # def auto_choice(self):
    #     """
    #     Allocate devices for multi-GPU running.
    #     """
    #     if self.gpus is None:
    #         return ['cpu']
    #     elif self.specified_device >= 0:
    #         return ['cuda:{}'.format(self.specified_device)]
    #     else:
    #         available_gpus = [gpu for gpu in self.devices if not gpu['allocated']]
    #         if not available_gpus:
    #             for gpu in self.gpus:
    #                 gpu['allocated'] = False
    #             available_gpus = self.devices
    #         for gpu in available_gpus:
    #             self.gpus[gpu['index']]['allocated'] = True
...
if __name__ == '__main__':
    ...
    gpu_manager = GPUManager(gpu_available=True)
    devices = gpu_manager.auto_choice()
