...
class RelTypeSplitter(BaseTransform, BaseSplitter):
    ...
    def __call__(self, data, **kwargs):
        ...
        train_ratio = data.train_mask.sum().item() / data.num_edges
        test_ratio = data.test_mask.sum().item() / data.num_edges
        for idx_j in idx_slice:
            edge_index = data.edge_index.T[idx_j].T
            edge_type = data.y[idx_j]
            train_edge_mask = data.train_mask[idx_j]
            valid_edge_mask = data.val_mask[idx_j]
            test_edge_mask = data.test_mask[idx_j]
            ...
            sub_g = Data(x=data.x,
                         ...
                         index_orig=torch.arange(data.num_nodes),
                         edge_type=edge_type,
